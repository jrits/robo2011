...
// シーソー開始座標
// 中間ゲート
float GPS_SEESAW_START_X = 2950.0;
...
// インコースガレージ・イン開始座標
#if INCOURSE_STRAIGHT_GARAGEIN // インコース専用直線検知ver(実験中)
// ゴール前直線の前直線上（階段開始の延長線と交わる座標）
float GPS_GARAGEIN_START_X = 4000.0;
float GPS_GARAGEIN_START_Y = -2070.0;
float GPS_GARAGEIN_START_DIRECTION = 360.0;
#else
...
#if INCOURSE_STRAIGHT_GARAGEIN // インコース専用直線検知ver(実験中)
Region GPS_GARAGEIN_START = MakeRegion(GPS_GARAGEIN_START_X, GPS_GARAGEIN_START_Y +250.0,GPS_GARAGEIN_START_X + 500.0, GPS_GARAGEIN_START_Y -250.0);
#else
...
Region GPS_GARAGEIN_START = MakeRegion(GPS_GARAGEIN_START_X - 250.0, GPS_GARAGEIN_START_Y, GPS_GARAGEIN_START_X + 250.0, GPS_GARAGEIN_START_Y - 500.0);
#endif
